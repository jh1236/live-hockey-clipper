...
default_config = [
    ['Altius Updater', lambda f: AltiusManager.update_altius_pages(force_regen=f)],
    ['Live Hockey Updater', lambda f: LiveHockeyManager.update_live_hockey()],
    ['Whistle IQ Updater', lambda f: WhistleIQManager.update_whistle_iq()]
]
...
async def populate_empty_database():
    ...
    flush()
    await update_database(force_regen=False)


async def update_database(config=None, force_regen=True):
    config = config if config is not None else default_config
    for [name, task] in config:
        logging.info(f'Beginning task "{name}"')
        try:
            await task(force_regen)
            flush()
            logging.info(f'Completed task "{name}"')
...
if __name__ == '__main__':
    ...
    asyncio.run(populate_empty_database())
